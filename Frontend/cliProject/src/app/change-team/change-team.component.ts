...
export class ChangeTeamComponent implements OnInit {
  ...
  constructor(private informNewTeamService: InformNewTeamService, private teamService: TeamService, private notService: NotificationService) {
    this.listTeams();
  }
  ...
  listTeams() {
    ...
    this.teamService.listTeams()
      .subscribe(
        ...
        error => {
          this.loading = false;
          this.notService.error('Team list error', error.message);
        });
  }

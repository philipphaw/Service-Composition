...
export class AuthSettingsComponent implements OnInit {
  ...
  constructor(private authenticationService: AuthenticationService, private notService: NotificationService) {
    this.fillAuths();
  }
  ...
  getAuthList() {
    ...
    this.authenticationService.authList()
      .subscribe(
        ...
        error => {
          this.notService.error('could not retrieve authentication status list', error.message);
          this.loading = false;
        });
      ...
      if (this.services[i].type === 'oauth2' && !this.services[i].status) {
        ...
        this.authenticationService.oauth2Link(this.services[i].name.toLowerCase())
          .subscribe(
            ...
            error => {
              this.notService.error('could not retrieve auth url for service: ' + this.services[i].name, error.message);
              this.services[i].loading = false;
            });
      }
    }
  ...
  sendBasicAuth(service: string, username: string, password: string) {
    ...
    this.authenticationService.setBasicAuthOfService(service, username, password)
      .subscribe(
        ...
        error => {
          target.loading = false;
          this.notService.error('could not set basicauth', error.message);
        });
  }

  ...
  getRepositories() {
    ...
    this.versionControlService.getRepositories(this.name)
      .subscribe(
        ...
        error => {
          this.repoListLoading = false;
          this.notService.error('Got versioncontrol  error', error.message);
        });
  }
  ...
  getRepoContent() {
    ...
    this.versionControlService.getRepositoryContent(this.parsePath(), this.selectedRepo, this.name)
      .subscribe(
        ...
        error => {
          this.notService.error('repo content error', error.message);
          this.loading = false;
        });
  }
          ...
          if (myUser.username !== memberName) {
            this.versionControlService.shareRepository(this.selectedRepo, this.name, memberName)
              .subscribe(
                ...
                error => {
                  this.notService.error('Could not share repo with user: ' + memberName, error.message);
                });
          }
        }
      }
